...
const SYMBOLS = ["NIFTY", "BANKNIFTY", "SENSEX", "GOLD", "GOLDM"];
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        // Returns the mapped exchange, or defaults to "MCX"
        const findExchange = exchangeMap[symbol] || "NFO";
    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      {!loading && futures.length === 0 && (
        <div className="p-8 text-center bg-slate-900/40 rounded-xl border border-slate-800 text-sm text-slate-500">
          No active FUTIDX contracts found for{" "}
          <span className="font-semibold text-slate-300">{symbol}</span>.
        </div>
      )}
